...
stopinfo:=false;
i:=5;
{$IFDEF MSWINDOWS}rc:=Form_gwrap.StringGrid1.Rowcount-5;{$ELSE}rc:=Form_gwrap.StringGrid1.Rowcount;{$ENDIF}
...
case modeofuse of
   ...
   2: begin
      ...
      {$IFDEF MSWINDOWS}//require 7z 9.35
      minline:=5;
      try
...
begin
if ufileexists(fget_usrtmp_path+STR_STOPALL) then
try
...
begin
uassignfile(pstopfile,fget_usrtmp_path+STR_STOPALL);
rewrite(pstopfile);
closefile(pstopfile);
stopped:=true;
end;
